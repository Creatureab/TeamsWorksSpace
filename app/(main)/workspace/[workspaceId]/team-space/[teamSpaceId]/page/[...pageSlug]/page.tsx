// ✓ FIXED: Team space hierarchical page route with MongoDB
...
export default async function TeamSpaceHierarchicalPage({
    params
}: {
    params: Promise<PageParams>
}) {
    const { workspaceId, teamSpaceId, pageSlug } = await params;
    const decodedTeamSpaceId = decodeURIComponent(teamSpaceId);
    const slugSegments = (Array.isArray(pageSlug) ? pageSlug : [pageSlug])
        .filter(Boolean)
        .map((segment) => decodeURIComponent(segment));

    try {
        // Get page by path segments within team space
        const page = await pageService.getPageByPath(workspaceId, slugSegments, decodedTeamSpaceId);
                    ...
                    <nav className="mb-6">
                        <ol className="flex items-center space-x-2 text-sm text-gray-500">
                            ...
                            <li className="flex items-center">
                                ...
                                <Link
                                    href={`/workspace/${workspaceId}/team-space/${encodeURIComponent(decodedTeamSpaceId)}`}
                            ...
                            {pathSegments.map((segment: string, index: number) => {
                                ...
                                const href = `/workspace/${workspaceId}/team-space/${encodeURIComponent(
                                    decodedTeamSpaceId
                                )}/page/${page.path
                                    .slice(0, index + 1)
        ...
        console.error('Error loading team space page:', error);
        notFound();
    }
}
...
export async function generateMetadata({ params }: { params: Promise<PageParams> }) {
    try {
        ...
        const decodedTeamSpaceId = decodeURIComponent(resolved.teamSpaceId);
        ...
        const page = await pageService.getPageByPath(resolved.workspaceId, slugSegments, decodedTeamSpaceId);
