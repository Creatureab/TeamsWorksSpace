...
export default async function TeamSpaceProjectsDatabasePage({
  params,
}: {
  params: Promise<{ workspaceId: string; teamSpaceId: string }>;
}) {
  const { workspaceId, teamSpaceId } = await params;
  const data = await getWorkspaceViewData({ workspaceId, teamSpaceId });
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Team space database view (Notion-style table).
            </p>
          </div>
              ...
              {(!data.projects || data.projects.length === 0) && (
                <TableRow>
                  <TableCell colSpan={3} className="py-8 text-center text-sm text-slate-500">
                    No projects in this team space yet.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
